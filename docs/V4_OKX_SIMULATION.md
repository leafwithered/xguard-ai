...
Four real read-only requests were completed through the protected Vercel Preview after the project owner configured Preview-scoped credentials. No wallet was connected, no transaction was signed or broadcast, and no chain state was changed.
...
| V4.1 exact-deployment validation | same public approval fixture | `200` / code `0` / `AVAILABLE` | `Token Approval` | none | `358 ms` |

Live sample count is `4`. Median simulation latency is `376 ms`; the slowest observed sample is `492 ms`. No p95 is claimed from four samples. The exact V4.1 code deployment was `xguard-4h380w1l6-leafwithereds-projects.vercel.app`; its validation returned final `25 LOW`, deterministic `20 LOW`, Hybrid Analysis, `LOW` confidence, `UNDETERMINED`, and `CONSISTENT`. The approval fixture is exposed in Judge Mode because it is public, reproducible, meaningful, and returned successful read-only provider evidence. The transfer fixture remains documentation-only because its current-state revert is useful failure evidence but not the clearest primary demo.
