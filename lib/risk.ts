...
function signal(id: string, title: string, detail: string, severity: RiskSignal["severity"] = "critical"): RiskSignal {
...
export function localRiskAnalysis(input: RiskInput): RiskResult {
  ...
  if (input.to.toLowerCase() === zeroAddress) {
    ...
    criticalSignals.push(signal("zero-address", "Zero-address destination", "The recipient is the burn/null address."));
    score += 62;
  }
  if (decodedAction.status === "malformed") {
    reasons.push("Calldata is malformed for the detected contract method");
    criticalSignals.push(signal("malformed-calldata", "Malformed calldata", decodedAction.riskHint ?? "ABI decoding failed."));
    score += 18;
  }
  if (decodedAction.status === "unknown") {
    reasons.push("Unknown contract method selector");
    advisorySignals.push(signal("unknown-method", "Unknown method", decodedAction.riskHint ?? "The method is not in the built-in decoder.", "advisory"));
    score += 22;
  }
  if (decodedAction.action === "ERC20 Approval") {
    reasons.push("ERC20 approve grants token spending permission");
    criticalSignals.push(signal("erc20-approval", "ERC20 approval", decodedAction.spender ? `Spender: ${decodedAction.spender}` : "A spender address is encoded."));
    score += 30;
    if (decodedAction.isUnlimited) {
      reasons.push("Unlimited ERC20 approval amount detected");
      criticalSignals.push(signal("unlimited-approval", "Unlimited approval", "The spender can move the full token balance."));
      score += 34;
    }
  }
  if (decodedAction.action === "NFT Operator Approval") {
    ...
    const operatorSignal = signal("operator-approval", decodedAction.approved ? "NFT operator permission" : "NFT operator revocation", decodedAction.operator ? `Operator: ${decodedAction.operator}` : "An operator address is encoded.", decodedAction.approved ? "critical" : "advisory");
  ...
  if (decodedAction.action === "ERC20 Transfer" || decodedAction.action === "ERC20 Transfer From") {
    advisorySignals.push(signal("token-transfer", "Token transfer", decodedAction.amount ? `Token amount: ${decodedAction.amount}` : "An ERC20 transfer amount is encoded.", "advisory"));
  }
  if (input.data === "0x") {
    reasons.push("No calldata: this appears to be a native token transfer");
    advisorySignals.push(signal("empty-calldata", "Native transfer", "No contract method calldata was supplied.", "advisory"));
  }
  if (valueWei >= parseUnits("10", 18)) {
    reasons.push("Unusually large native token value");
    criticalSignals.push(signal("large-value", "Large native value", `Value: ${input.value} OKB`));
    score += 28;
  } else if (valueWei >= parseUnits("1", 18)) {
    reasons.push("Material native token value");
    advisorySignals.push(signal("material-value", "Material native value", `Value: ${input.value} OKB`, "advisory"));
    score += 12;
  }
  if (text.includes("new contract") || text.includes("unverified") || text.includes("unknown")) {
    ...
    advisorySignals.push(signal("unknown-contract", "Unknown contract context", "The context describes an unknown or unverified contract.", "advisory"));
    score += 18;
  }
  if (text.includes("airdrop") || text.includes("claim") || text.includes("urgent")) {
    ...
    advisorySignals.push(signal("social-engineering", "Social-engineering signal", "Urgency or airdrop/claim language can pressure users.", "advisory"));
    score += 16;
  }
